        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <button
              ...
              style={{ height: 34, padding: '0 12px', borderRadius: 8, border: '1px solid rgba(196,106,45,0.4)', background: 'transparent', color: 'var(--action)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}
            >
              ↓ Lista
            </button>
            <button
              ...
              {items.map(a => (
                <button
                  ...
                  style={{
                    ...
                    background:   selected?.id === a.id ? 'rgba(196,106,45,0.06)' : 'var(--surface)',
                    cursor:       'pointer',
                  }}
                >
    ...
    <div style={{ background: 'var(--primary)', display: 'flex', flexDirection: 'column', height: '100%', overflowY: mode === 'bulk-import' ? 'hidden' : 'auto', padding: mode === 'bulk-import' ? 0 : '24px 20px' }}>
      ...
      {mode === 'idle' && (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <p style={{ color: 'rgba(242,233,220,0.3)', fontSize: 14 }}>Seleciona ou cria um artigo</p>
        </div>
      )}
    </div>
  )
